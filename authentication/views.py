...
from authentication import signals, utils
from authentication.compat import get_user_email
from authentication.conf import settings
from .models import Influencer, BrandUser
from .serializers import TokenObtainPairSerializer, InfluencerSerializer, BrandUserSerializer, \
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        influencer = Influencer.objects.filter(user_ptr_id=instance.id).first()
        if influencer:
            serializer = InfluencerDetailSerializer(influencer)
            return Response(serializer.data)

        brand_user = BrandUser.objects.filter(user_ptr_id=instance.id).first()
        if brand_user:
            serializer = BrandUserSerializer(brand_user)
            return Response(serializer.data)
